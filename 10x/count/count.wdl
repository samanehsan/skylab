...
task bucket_by_bc_main {
  ...
  command <<<
    ...
    echo "$(jq --arg var "$(<{read_groups})" '. + {"read_groups": $var}' _args)" > _args
...
workflow count {
  ...
  call setup_chunks {
    input: sample_id=sample_id, sample_def=sample_def, read_path=read_path
  }

  call chunk_reads_split {
    input: chunks_setup=setup_chunks.chunks_setup, reads_per_file=reads_per_file
  }

  scatter(chunk in chunk_reads_split.chunks) {
    call chunk_reads_main {
      input: chunk=chunk, chunks=setup_chunks.chunks_setup, reads_per_file=reads_per_file, read_path=read_path
    }
  }

  call chunk_reads_join {
    input: outs=chunk_reads_main.outs
  }

  call extract_reads_split {
    input: out_chunks=chunk_reads_join.out_chunks, barcode_whitelist=setup_chunks.barcode_whitelist
  }
  ...
  scatter(split_pair in zip(extract_reads_split.chunks, chunk_reads_main.fastq_chunks)) {
      call extract_reads_main {
        input: out_chunks=chunk_reads_join.out_chunks, barcode_whitelist=setup_chunks.barcode_whitelist,
          ...
          chemistry_def=setup_chunks.chemistry_def, chunk=split_pair.left,
  ...
  call extract_reads_join {
    input: align=align, barcode_whitelist=setup_chunks.barcode_whitelist, chemistry_def=setup_chunks.chemistry_def,
      outs=extract_reads_main.outs, stage_defs=extract_reads_split.stage_defs, reads_from_mains=extract_reads_main.reads,
      trimmed_seqs_from_mains=extract_reads_main.trimmed_seqs, chunked_reporter_from_mains=extract_reads_main.chunked_reporter,
      barcode_counts_from_mains=extract_reads_main.barcode_counts
  }

  call align_reads_split {
    input: read_groups=extract_reads_join.read_groups, read_paths=extract_reads_join.read_paths,
      reference_path=reference_path
  }

  scatter(split_pair in zip(align_reads_split.chunks, extract_reads_join.reads)) {
    call align_reads_main {
      input: reference_path=reference_path, chunk=split_pair.left, chunked_fastq=split_pair.right,
        max_hits_per_read=-1
    }
  }
  ...
  call attach_bcs_and_umis_join {
    input: reference_path=reference_path, num_alignments_from_mains=attach_bcs_and_umis_main.num_alignments,
      output_from_mains=attach_bcs_and_umis_main.output_bam, chunked_reporter_from_mains=attach_bcs_and_umis_main.chunked_reporter,
      outs=attach_bcs_and_umis_main.outs
  }

  call bucket_by_bc_split {
    input: inputs=attach_bcs_and_umis_main.output_bam, num_alignments=attach_bcs_and_umis_main.num_alignments
  }
  ...
  scatter(chunk in mark_duplicates_split.chunks) {
    call mark_duplicates_main {
      input: input_bam=sort_by_bc_join.default, chunk=chunk, align=extract_reads_join.out_align, reference_path=reference_path
    }
  }

  call mark_duplicates_join {
    input: output_bam_from_mains=mark_duplicates_main.output_bam, chunked_reporter_from_mains=mark_duplicates_main.chunked_reporter,
  ...
  scatter(chunk in count_genes_split.chunks) {
    ...
    input: bam_inputs=mark_duplicates_main.output_bam, barcode_whitelist=setup_chunks.barcode_whitelist, gem_groups=extract_reads_join.gem_groups,
      reference_path=reference_path, barcode_summary=attach_bcs_and_umis_join.barcode_summary, chemistry_def=setup_chunks.chemistry_def,
      chunk=chunk, align=extract_reads_join.out_align
    }
  }

  call count_genes_join {
    input: matrices_h5_from_mains=count_genes_main.matrices_h5, chunked_reporter_from_mains=count_genes_main.chunked_reporter,
